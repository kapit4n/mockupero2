 ...
 */
angular.module("mockuperApp").controller("MockupEditDesignCtrl", [
  "$scope",
  "$rootScope",
  "loginService",
  "$compile",
  ...
  function (
                        ...
                        .$promise.then(
                          function (resultItems) {
                            ...
                            mockupService.saveMockupItem.save(
                              resultItems[0],
                              function (resultItemsSaved) {
                                console.log(resultItemsSaved);
                                $scope.loadMockupItems();
                              }
                            );
                          },
    ...
    $scope.createImage = function () {
      html2canvas($("#design-div"), {
        onrendered: function (canvas) {
          ...
          console.log("load image here lllllllllllllllll");
    ...
    $scope.loadStaticValues = function (item) {
      if (
        item.id &&
        $scope.mockupItemsBykey[item.id] &&
        $scope.mockupItemsBykey[item.id].link
      ) {
    ...
    $scope.updateInput = function (labelId, idComponent) {
      switch (labelId) {
        ...
        case "height":
          ...
          break;
        case "top":
          var component = angular.element(
            document.querySelector("#" + idComponent)
          );
          var translateY = parseInt(
            $(component[0]).css("transform").split(",")[5]
          );
          var textValue = angular.element(
            document.querySelector("#" + labelId)
          );
          if (translateY) {
            $("#" + idComponent)[0].style.top =
              parseFloat(textValue[0].value) - translateY + "px";
          } else {
            $("#" + idComponent)[0].style.top =
              parseFloat(textValue[0].value) + "px";
          }
          break;
        case "left":
          var component = angular.element(
          ...
          var translateX = parseInt(
            $(component[0]).css("transform").split(",")[4]
          );
          var textValue = angular.element(
            document.querySelector("#" + labelId)
          );
          if (translateX) {
            $("#" + idComponent)[0].style.left =
              parseFloat(textValue[0].value) - translateX + "px";
          } else {
            $("#" + idComponent)[0].style.left =
              parseFloat(textValue[0].value) + "px";
          }
    ...
    $scope.loadProperties = function (idComponent) {
      var propertiesDiv = angular.element(
      ...
      if (idComponent.indexOf("image") > -1) {
        myComponent =
          ...
          propertyService.image(idComponent, $scope) +
          "</div>";
      } else if (idComponent.indexOf("button") > -1) {
        myComponent =
          '<div class="alert" id="wrapper" style="z-index: 100;"> <span class="close" data-dismiss="alert">X</span>' +
          propertyService.button(idComponent, $scope) +
          "</div>";
      } else if (idComponent.indexOf("input") > -1) {
        myComponent =
          '<div class="alert" id="wrapper" style="z-index: 100;"> <span class="close" data-dismiss="alert">X</span>' +
          propertyService.input(idComponent, $scope) +
          "</div>";
      } else if (idComponent.indexOf("label") > -1) {
        myComponent =
          '<div class="alert" id="wrapper" style="z-index: 100;"> <span class="close" data-dismiss="alert">X</span>' +
          propertyService.label(idComponent, $scope) +
          "</div>";
    ...
    $scope.deleteItem = function (idComponent) {
      ...
      if (idComponent.indexOf("new-") > -1) {
        ...
        mockupService.deleteMockupItem
          .deleteIt({
            id: itemId,
          })
          .$promise.then(function (result) {
    ...
    io.socket.on("mockupversion", function (msg) {
      $scope.$apply(function () {
        if (
          msg.data.mockup == $routeParams.mockupId ||
          msg.data.mockup == $routeParams.suggestId
        ) {
          var message =
            '<span id="alert_message_text">' + msg.data.message + " </span>";
          $.notify(message, {
            newest_on_top: true,
          });
    ...
    $scope.reloadMockupVersions = function () {
      mockupVersionService.getMockupVersions
        .get({
          /* where: {
    ...
    $scope.deleteVersion = function (versionId) {
      mockupVersionService.deleteMockupVersion
        .save({
    ...
    $scope.restore = function (versionId) {
      mockupVersionService.mockupVersionRestore
        .save({
          mockupVersionId: versionId,
          action: "Restoring",
        })
